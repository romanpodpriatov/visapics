...
def remove_background_and_make_white(image, ort_session, target_color_rgb: Tuple[int, int, int] = (255, 255, 255), return_mask: bool = False):
    ...
    """
    try:
        logging.info(f"Starting background removal for image size: {image.size}")
        ...
        logging.info(f"Prepared input tensor shape: {img.shape}")
        logging.info(f"Model input name: {ort_session.get_inputs()[0].name}")
        logging.info(f"Model input shape: {ort_session.get_inputs()[0].shape}")

        # Run through model
        ort_inputs = {ort_session.get_inputs()[0].name: img}
        logging.info("Running ONNX inference...")
        ort_outs = ort_session.run(None, ort_inputs)
        logging.info(f"ONNX inference completed. Output shape: {ort_outs[0].shape}")
        ...
        logging.info(f"Processed prediction shape: {pred.shape}")
